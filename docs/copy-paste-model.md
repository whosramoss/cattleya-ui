...
`npm install @cattleya-ui/react` brings the design tokens (`styles.css`) and every runtime dependency each component needs (`@radix-ui/*`, `motion`, `tailwind-variants`, …) — it does **not** expose components for direct import. `@cattleya-ui/react/components/button` is not a valid subpath; only `./styles.css` is exported. This is intentional:
...
2. Find the component in Storybook, on [GitHub](https://github.com/whosramoss/cattleya-ui/tree/main/packages/react/src/components), or in `node_modules/@cattleya-ui/react/src/components`.
